...
contract OwnerFundedPaymaster is Ownable, ReentrancyGuard {
    ...
    event TransactionSponsored(
        address indexed user, address indexed target, uint256 gasUsed, uint256 fee, bool paidByOwner
    );
     ...
     */
    function sponsorTransaction(IEIP2771Forwarder.ForwardRequest calldata req, bytes calldata signature)
        external
        nonReentrant
     ...
     */
    function canSponsorTransaction(address user, address target, uint256 gasLimit) external view returns (bool) {
     ...
     */
    function processPayment(address user, address target, uint256 gasUsed, uint256 gasPrice) public onlyForwarder {
