...
library PaymasterUtils {
     ...
     */
    function calculateEstimatedFee(uint256 gasLimit, uint256 gasPrice, uint256 baseFee, uint256 feeMultiplier)
        internal
        pure
        returns (uint256)
    {
    ...
    function validateSponsorshipParams(
        ...
        return user != address(0) && target != address(0) && gasLimit > 0 && gasLimit <= maxGasLimit
     ...
     */
    function hasSufficientCredits(address user, uint256 requiredAmount, mapping(address => uint256) storage userCredits)
        internal
        view
        returns (bool)
    {
     ...
     */
    function isTokenWhitelisted(address token, mapping(address => bool) storage whitelistedTokens)
        internal
        view
        returns (bool)
    {
