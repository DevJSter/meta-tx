...
contract PaymasterUtilsTest is Test {
    ...
    function callCalculateEstimatedFee(uint256 gasLimit, uint256 gasPrice, uint256 baseFee, uint256 feeMultiplier)
        external
        pure
        returns (uint256)
    {
        return PaymasterUtils.calculateEstimatedFee(gasLimit, gasPrice, baseFee, feeMultiplier);
    }

    /// Wraps validateSponsorshipParams(...) which reads from sponsoredContracts
    function callValidateSponsorshipParams(address user, address target, uint256 gasLimit, uint256 maxGasLimit)
        external
        view
        returns (bool)
    {
        return PaymasterUtils.validateSponsorshipParams(user, target, gasLimit, maxGasLimit, sponsoredContracts);
    }

    /// Wraps hasSufficientCredits(...) which reads from userCredits
    function callHasSufficientCredits(address user, uint256 requiredAmount) external view returns (bool) {
        return PaymasterUtils.hasSufficientCredits(user, requiredAmount, userCredits);
    }

    /// Wraps isTokenWhitelisted(...) which reads from whitelistedTokens
    function callIsTokenWhitelisted(address token) external view returns (bool) {
        return PaymasterUtils.isTokenWhitelisted(token, whitelistedTokens);
    }
    ...
    function testHasSufficientCredits_None() public {
        ...
        assertFalse(this.callHasSufficientCredits(address(0x13), 1));
    }
