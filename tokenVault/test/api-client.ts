...
export class TokenVaultClient {
  ...
  async checkWalletExists(aadhaarNumber: string) {
    try {
      const response = await axios.get(`${this.baseURL}/wallet/exists?aadhaar=${aadhaarNumber}`);
      return response.data;
    } catch (error) {
      throw this.handleError(error);
    }
  ...
  async getSignatureHistory(aadhaarNumber: string) {
    try {
      const response = await axios.get(`${this.baseURL}/wallet/signatures?aadhaar=${aadhaarNumber}`);
      return response.data;
    } catch (error) {
      throw this.handleError(error);
    }
...
export async function testAPIClient() {
  ...
  try {
    ...
    console.log('PASS: Signature valid:', verification.data.isValid);

    // Get signature history
    console.log('\n6. Get Signature History');
    const history = await client.getSignatureHistory(aadhaarNumber);
    console.log('PASS: Signature history:', history.count, 'signatures');

    // Get all wallets (admin)
    console.log('\n7. Get All Wallets (Admin)');
    const allWallets = await client.getAllWallets();
    console.log('PASS: Total wallets:', allWallets.count);

    console.log('\nAll API tests passed!');

  } catch (error) {
    console.error('FAIL: API test failed:', error instanceof Error ? error.message : String(error));
  }
}
